...
const Register = () => {
  ...
  const handleSubmit = async e => {
    e.preventDefault()
    if (e.target.name.value === "" && e.target.email.value === "" && e.target.pass.value === "" && e.target.repass.value === "") {
      return toast.error('All fields must be completed', { position: 'top-right', icon: '⚠️' });
    }
    if (e.target.pass.value !== e.target.repass.value) {
      return toast.error('Passwords must be the same', { position: 'top-right', icon: '⚠️' });
    }
    if (e.target.pass.value.length < 6) {
      return toast.error('Password must have at least 6 characters', { position: 'top-right', icon: '⚠️' });
    }

    try {
      const res = await createUserWithEmailAndPassword(auth, e.target.email.value, e.target.repass.value)
      ...
      console.log(error);
      return toast.error('Server failed', { position: 'top-right', icon: '⚠️' });
    }
  }
              ...
              <div className="flex -mx-3">
                <div className="w-full px-3 mb-5">
                  ...
                  <div className="flex">
                    ...
                    <input type="text" name='name' className="w-full -ml-10 px-2 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="johnsmith@example.com" />
                  </div>
                </div>
              </div>
              <div className="flex -mx-3">
                <div className="w-full px-3 mb-5">
                  ...
                  <div className="flex">
                    <div className="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><i className="mdi mdi-email-outline text-gray-400 text-lg"></i></div>
                    <input type="email" name='email' className="w-full -ml-10 px-2 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="johnsmith@example.com" />
                  </div>
                </div>
              </div>
              <div className="flex -mx-3">
                <div className="w-1/2 px-3 mb-5">
                  ...
                  <div className="flex">
                    ...
                    <input type="password" name='pass' className="w-full -ml-10 px-2 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="John" />
                  </div>
                </div>
                <div className="w-1/2 px-3 mb-5">
                  ...
                  <div className="flex">
                    <div className="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><i className="mdi mdi-account-outline text-gray-400 text-lg"></i></div>
                    <input type="password" name='repass' className="w-full -ml-10 px-2 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="Smith" />
                  </div>
                </div>
              </div>
